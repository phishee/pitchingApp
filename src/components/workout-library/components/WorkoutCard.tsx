...
export function WorkoutCard({ workout, onView, onEdit, onAssign }: WorkoutCardProps) {
  const WorkoutIcon = getWorkoutIcon(workout.tags);
  const workoutColor = getWorkoutColor(workout.tags);

  return (
    <Card 
      className="group relative overflow-hidden border-0 shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 bg-white cursor-pointer"
    ...
    >
      ...
      <CardContent className="p-4 space-y-3">
        ...
        {/* Action Button */}
        <Button 
          className="w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white text-xs font-medium py-2 shadow-sm hover:shadow-md transition-all duration-200"
          onClick={(e) => {
            e.stopPropagation();
            onAssign(workout.id);
          }}
