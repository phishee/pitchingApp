...
import { ExercisePrescriptionStepProps } from '../work-assignment-types';
...
export function Step3ExercisePrescriptions({
  ...
  // Get exercise prescriptions from context
  const exercisePrescriptions = workoutDetails?.exercisePrescriptions || {};
  ...
  const handleMetricChange = (exerciseId: string, metricKey: string, value: any) => {
    const currentPrescription = exercisePrescriptions[exerciseId];
  ...
  const handleNotesChange = (exerciseId: string, notes: string) => {
    updateExercisePrescription(exerciseId, { notes });
  };
  ...
  const getModifiedCount = () => {
    return Object.values(exercisePrescriptions).filter(prescription => prescription.isModified).length;
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {isLoadingExercises ? (
          ...
          selectedWorkout.flow.exercises.map((exercise, index) => {
            ...
            const prescription = exercisePrescriptions[exercise.exercise_id];
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {exerciseDetails?.tags && exerciseDetails.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1">
                          {exerciseDetails.tags.slice(0, 2).map(tag => (
                            <Badge key={tag} variant="outline" className="text-xs">
                              {tag}
                            </Badge>
                          ))}
                          {exerciseDetails.tags.length > 2 && (
                            <Badge variant="outline" className="text-xs">
                              +{exerciseDetails.tags.length - 2}
                            </Badge>
                          )}
      ...
      {/* Validation */}
      {selectedWorkout.flow.exercises.length === 0 && (
        <div className="flex items-center gap-2 p-3 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900 rounded-lg text-amber-900 dark:text-amber-200 text-sm">
          <AlertCircle className="h-4 w-4 flex-shrink-0" />
          <p>This workout has no exercises to prescribe</p>
        </div>
      )}
    </div>
  );
