...
const TAEM_MEMBER_API_BASE = "/team-members";
const TEAM_INVITATION_API_BASE = "/team-invitations";
const TEAM_REQUEST_API_BASE = "/team-requests";
const TEAM_API_BASE = "/teams";

export const teamMemberApi = {
  // Get all team members
  async getTeamMembers(): Promise<TeamMember[]> {
    const res = await apiClient.get<TeamMember[]>(TAEM_MEMBER_API_BASE);
    return res.data;
  },

  // Get a single team member by ID
  async getTeamMember(id: string): Promise<TeamMember> {
    const res = await apiClient.get<TeamMember>(`${TAEM_MEMBER_API_BASE}/${id}`);
    return res.data;
  },

  // Create a new team member
  async createTeamMember(data: Partial<TeamMember>): Promise<TeamMember> {
    const res = await apiClient.post<TeamMember>(TAEM_MEMBER_API_BASE, data);
    return res.data;
  },

  // Update a team member by ID
  async updateTeamMember(id: string, data: Partial<TeamMember>): Promise<TeamMember> {
    const res = await apiClient.put<TeamMember>(`${TAEM_MEMBER_API_BASE}/${id}`, data);
    return res.data;
  },

  // Delete a team member by ID
  async deleteTeamMember(id: string): Promise<{ message: string }> {
    const res = await apiClient.delete<{ message: string }>(`${TAEM_MEMBER_API_BASE}/${id}`);
    return res.data;
  },

  // Get all team members for a specific team
  async getTeamMembersByTeam(teamId: string): Promise<TeamMember[]> {
    const res = await apiClient.get<TeamMember[]>(`${TAEM_MEMBER_API_BASE}/team/${teamId}`);
    return res.data;
  },

  // Get all team members for a specific user
  async getTeamMembersByUser(userId: string): Promise<TeamMember[]> {
    const res = await apiClient.get<TeamMember[]>(`${TAEM_MEMBER_API_BASE}/user/${userId}`);
    return res.data;
  },

  // Get team members by status
  async getTeamMembersByStatus(status: 'active' | 'inactive'): Promise<TeamMember[]> {
    const res = await apiClient.get<TeamMember[]>(`${TAEM_MEMBER_API_BASE}/status/${status}`);
    return res.data;
  },

  // Get team invitations by team ID
  async getTeamInvitationsByTeam(teamId: string): Promise<TeamInvitationWithTeamUserInfo[]> {
    const res = await apiClient.get<TeamInvitationWithTeamUserInfo[]>(`${TEAM_API_BASE}/${teamId}/invitations`);
    return res.data;
  },

  // Get team requests by team ID
  async getTeamRequestsByTeam(teamId: string): Promise<TeamJoinRequestWithTeamUserInfo[]> {
    const res = await apiClient.get<TeamJoinRequestWithTeamUserInfo[]>(`${TEAM_API_BASE}/${teamId}/join-requests`);
    return res.data;
  },

  // accept team request by id
  async acceptTeamRequest(requestId: string, teamId: string): Promise<TeamJoinRequestWithTeamUserInfo> {
    const res = await apiClient.put<TeamJoinRequestWithTeamUserInfo>(`${TEAM_API_BASE}/${teamId}/join-requests/${requestId}/approve`);
    return res.data;
  },
  ...
  async rejectTeamRequest(id: string, data: { comment?: string; rejectedBy?: string }): Promise<TeamJoinRequestWithTeamUserInfo> {
    const res = await apiClient.put<TeamJoinRequestWithTeamUserInfo>(
