...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const events = await controller.getAssignmentEvents(id);
    
    return NextResponse.json(events);
  } catch (error) {
    console.error('Failed to fetch assignment events:', error);
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const result = await controller.updateAssignmentAndEvents(id, {
      eventUpdates: body.eventUpdates,
    ...
    console.error('Failed to update assignment events:', error);
