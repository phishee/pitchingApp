...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ teamId: string; requestId: string }> }) {
    const joinRequestController = container.get<TeamJoinRequestController>(TEAM_JOIN_REQUEST_TYPES.TeamJoinRequestController);
    return joinRequestController.rejectJoinRequest(req, { params: Promise.resolve({ teamId: (await params).teamId, id: (await params).requestId }) });
}